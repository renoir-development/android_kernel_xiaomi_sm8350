...
static int fts_get_fw_file_via_request_firmware(struct fts_upgrade *upg, char *fw_name)
{
	int ret = 0;
	const struct firmware *fw = NULL;
	u8 *tmpbuf = NULL;
 ...
 *  Return: return 0 if success, otherwise return error code
 *****************************************************************************/
static int fts_fwupg_get_fw_file(struct fts_upgrade *upg, char *fw_name)
{
	int ret = 0;
	...
	if (FTS_FW_REQUEST_SUPPORT) {
		ret = fts_get_fw_file_via_request_firmware(upg, fw_name);
		if (ret != 0) {
			get_fw_i_flag = true;
		}
	} else {
...
int fts_fwupg_by_name(char *fw_name)
{
	...
	/* get fw */
	ret = fts_fwupg_get_fw_file(upg, fw_name);
	if (ret < 0) {
...
static void fts_fwupg_work(struct work_struct *work)
{
	...
	/* get fw */
	ret = fts_fwupg_get_fw_file(upg, NULL);
	if (ret < 0) {
		FTS_ERROR("get file fail, can't upgrade");
	} else {
		/* ic init if have */
